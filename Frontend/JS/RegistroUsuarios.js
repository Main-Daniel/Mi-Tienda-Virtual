    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (i, item) {
                ...
                tr.append($("<td></td>").text(item.direccion_cli));
...
function validateFields() {
    if ($("#txtCedula").val().trim().length === 0) {
        ...
        return false;
    } else if ($("#txtDireccion").val().trim().length === 0) {
...
function sendData() {
    if (!validateFields()) {
        ...
        let url = '/TiendaVirtualSB/registrarUsuario';
        let params = "cedula=" + cedula + "&" + "nombre=" + nombre + "&" + "direccion=" + direccion + "&" + 
